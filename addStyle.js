...
const calcBodyClass = "max-w-lg rounded overflow-hidden shadow-2x1 bg-pink-700 bg-opacity-75 bg-gradient-to-tr from-gray-800 flex";
...
const btnClass = "flex w-12 h-12 justify-center items-center m-1 font-bold py-2 px-2 border hover:border-transparent rounded text-white border-2 text-2x1 hover:bg-green-500 border-green-500";
const btnNumber = document.querySelector('.number');
btnClass.split(" ").forEach((cls) => btnNumber.classList.add(cls));
...
function addClasses(button) {
    ...
    const btnDeciamlClass = "hover:bg-yellow-500 border-yellow-500";
    ...
    if (button.classList.value === "number") {
        ...
        btnDeciamlClass.split(" ").forEach((cls) => button.classList.add(cls));
    }
